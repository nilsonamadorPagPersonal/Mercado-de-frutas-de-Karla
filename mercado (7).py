...
def init_db():
    ...
    # Sin productos demo - el dueño agrega los suyos desde el admin
    db.commit()
    db.close()
...
def imagen_producto(pid):
    db  = get_db()
    row = db.execute("SELECT imagen FROM productos WHERE id=?", (pid,)).fetchone()
    db.close()
    ...
    return Response(b64mod.b64decode(b64), mimetype=mime,
...
def tienda():
    ...
    for p in prods:
        ...
        img_src  = "/imagen/%d" % p["id"] if has_img else ""
        img_card = '<img src="%s" style="width:100%%;height:100%%;object-fit:cover" loading="lazy">' % img_src if has_img else "🍊"
        img_modal = ('<img src="%s" style="width:100%%;max-height:250px;object-fit:cover;border-radius:8px;margin-bottom:14px" loading="lazy">' % img_src) if has_img else '<div style="text-align:center;font-size:5rem;padding:18px 0">🍊</div>'
...
def admin_add_prod():
    ...
    if "imagen" in request.files:
        ...
        if fi and fi.filename:
            ...
            if len(raw) < 5_000_000: img = mime + ";" + base64.b64encode(raw).decode()
    db = get_db()
